  ...
  return (
    <nav className="sticky top-0 z-50 w-full border bg-background/80 backdrop-blur-md">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="ml-10 flex items-baseline space-x-8">
              {navigation.map(item => (
                <Link
                  key={item.name}
                  href={item.href}
                  className="text-muted-foreground hover:text-accent-foreground px-3 py-2 text-sm font-medium transition-colors duration-200 hover:bg-accent rounded-md"
                >
                  {item.name}
                </Link>
              ))}
        ...
        >
          <div className="px-2 pt-2 pb-3 space-y-1 border-primary">
            {navigation.map(item => (
              <Link
                key={item.name}
                href={item.href}
                className="text-muted-foreground hover:text-accent-foreground block px-3 py-2 text-base font-medium transition-colors duration-200 hover:bg-accent rounded-md"
